...
OpenProblems tasks contain specifications for the common config fields between the same component types. These specifications are defined in the `src/api/comp_*.yaml` files. For more information on how these specifications are formatted, see ["Design the API"](/create_task/design_api.qmd#step-3).
